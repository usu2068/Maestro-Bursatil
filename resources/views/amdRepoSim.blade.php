<div class="card pd-20 pd-sm-40">
  <h6 class="card-body-title">Reporte de uso de los simuladores.</h6>
  <p class="mg-b-20 mg-sm-b-30">En este modulo usted puede realizar un reporte de uso del los simuladores deseados en el intervalo de fechas que indique.</p>
  
  <div class="table-wrapper">

      <input id="urlRepoSimOpc" type="hidden" value="{{ url('/admin/reportes/simulador/repoOpc') }}">
      <div id="divMsj"></div>
      ...
      <div class="form-group">
        ...
        <select class="form-control select2" id="selSim">
      ...
      <div class="form-group">
        <a href="javascript:RepoSimOpc()" class="btn btn-info"><i class="fa fa-save tx-20"></i> Generar Reporte</a>
      </div>
    ...
    <table id="datatable1" class="table display responsive ">
      ...
      <tbody>
        ...
        @foreach($logsGene as $logGene)
          ...
          <tr>
            <td>
              <a href="javascript:detaRepoSim({{ $logGene->id }}, {{ $logGene->usuarios->id }}, {{ $logGene->simuladores->id }})" class="btn btn-info">
                <i class="fa fa-address-card tx-20"></i>
              </a>
            </td>
            <td>{{ $logGene->usuarios->name }}</td>
            <td>{{ $logGene->simuladores->nombre }}</td>
            <td>{{ round($logGene->efectividadTotal, 2) }} %</td>
            <td>{{ $logGene->fechaPresentacion }}</td>
          </tr>
          @else
